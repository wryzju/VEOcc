    ...
    with torch.no_grad():
        for i_iter_val, data in enumerate(val_dataset_loader):
            ...
            if args.save_ply:
                ...
                save_voxel_label = voxel_label.clone()
                save_voxel_predict[:, -1, :, :] = 12
                save_voxel_predict[:, :, -1, :] = 12
                save_voxel_predict[~fov_mask] = 12
                save_voxel_label[~fov_mask] = 12
                ...
                save_occ_label_ply(save_voxel_label.detach().cpu(), cfg.voxel_size, voxel_origin, save_gt_dir, sample_name)
            ...
            tp_sum, fp_sum, fn_sum = CalMeanIou.add_batch(voxel_predict, voxel_label)

            if args.record_sample_metrics:
                tp_sum = tp_sum.astype(np.float64)
                fp_sum = fp_sum.astype(np.float64)
                fn_sum = fn_sum.astype(np.float64)
                iou_ssc = tp_sum / (tp_sum + fp_sum + fn_sum + 1e-5)
                sample_miou = float(np.mean(iou_ssc[1:]))

                pred_np = voxel_predict.numpy()
                label_np = voxel_label.numpy()
                valid_mask = label_np != 255
                comp_tp, comp_fp, comp_fn = CalMeanIou.get_score_completion(pred_np, label_np, valid_mask)
                comp_denom = comp_tp + comp_fp + comp_fn
                sample_iou = float(comp_tp / comp_denom) if comp_denom > 0 else 0.0

                sample_name = metas[0].get('name', f'iter_{i_iter_val:06d}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-ply', action='store_true', help='save predicted and gt occupancy as ply under work_dir')
