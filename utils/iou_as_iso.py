...
def sync_ssc_metrics(metric, device):
    ...
    metric.completion_tp = float(scalar_tensor[0].item())
    metric.completion_fp = float(scalar_tensor[1].item())
    metric.completion_fn = float(scalar_tensor[2].item())

    tps_tensor = torch.as_tensor(metric.tps, dtype=torch.float64, device=device)
    fps_tensor = torch.as_tensor(metric.fps, dtype=torch.float64, device=device)
    fns_tensor = torch.as_tensor(metric.fns, dtype=torch.float64, device=device)
    dist.all_reduce(tps_tensor, op=dist.ReduceOp.SUM)
    dist.all_reduce(fps_tensor, op=dist.ReduceOp.SUM)
    dist.all_reduce(fns_tensor, op=dist.ReduceOp.SUM)
    metric.tps = tps_tensor.cpu().numpy()
    metric.fps = fps_tensor.cpu().numpy()
    metric.fns = fns_tensor.cpu().numpy()
