...
from .modules.gaussian import generate_guassian_depth_target
...
class GeometryDepth_Net(BaseModule):
    ...
    def __init__(
        ...
        start, end, step = self.grid_config['dbound']
        num = int(round((end - start) / step))
        ds = (torch.arange(num, dtype=torch.float) * step + start).view(-1, 1, 1)
        ...
        self.use_depth_anything_pred = use_depth_anything_pred

        self.constant_std = 0.05

        self.depth_aggregation = DepthAggregation(embed_dims=32, out_channels=1, D=self.D)
    ...
    def get_klv_depth_loss(self, depth_labels, depth_preds):
        with autocast(enabled=False):
            depth_gaussian_labels, depth_values = generate_guassian_depth_target(depth_labels.float(),
                                                                                 self.downsample,
    ...
    def forward(self, input, depth_pred):
        ...
        if not self.use_depth_anything_pred:
            depth_pred = torch.zeros_like(depth_pred)

        _, stereo_volume = self.get_downsampled_gt_depth(depth_pred)
