...
class Attention(nn.Module):
    ...
    def __init__(self, embed_dims, kernel_size=5):
        ...
        self.neighbor_atttention = NeighborhoodCrossAttention2D(dim=embed_dims,
                                                                num_heads=1,
...
def convbn_2d(in_channels, out_channels, kernel_size, stride, pad):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=pad, bias=False),
                         build_norm_layer(norm_cfg, out_channels)[1])


def convbn_3d(in_channels, out_channels, kernel_size, stride, pad):
    return nn.Sequential(nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=pad, bias=False),
                         build_norm_layer(norm_cfg, out_channels)[1])


class StereoFeatNet(nn.Module):

    def __init__(self, in_channels, mid_channels, depth_channels, cam_channels):
        super().__init__()
        ...
        self.feat_conv = nn.Sequential(nn.Conv2d(mid_channels, depth_channels, kernel_size=1, stride=1, padding=0), )
...
class SimpleUnet(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.conv1 = nn.Sequential(convbn_2d(in_channels, in_channels * 2, 3, 2, 1), nn.ReLU(inplace=True))

        self.conv2 = nn.Sequential(convbn_2d(in_channels * 2, in_channels * 2, 3, 1, 1), nn.ReLU(inplace=True))

        self.conv3 = nn.Sequential(convbn_2d(in_channels * 2, in_channels * 4, 3, 2, 1), nn.ReLU(inplace=True))

        self.conv4 = nn.Sequential(convbn_2d(in_channels * 4, in_channels * 4, 3, 1, 1), nn.ReLU(inplace=True))
...
class CostVolumeEncoder(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, context_channels, cam_channels, dbound, downsample):
        ...
        self.UNet = nn.Sequential(SimpleUnet(D), SimpleUnet(D))
...
class ChannelAttention3D(nn.Module):
    ...
    def __init__(self, embed_dims):
        ...
        self.conv = nn.Sequential(nn.Conv3d(embed_dims, embed_dims, kernel_size=3, stride=1, dilation=1, padding=1, groups=1),
                                  nn.GELU(),
...
class DepthAggregation(nn.Module):
    ...
    def __init__(self, embed_dims=32, out_channels=1, D=96):
        super(DepthAggregation, self).__init__()

        self.stem = nn.Conv3d(2, embed_dims, kernel_size=3, stride=1, padding=1)
        self.mono_stereo_attention = Attention(D, kernel_size=5)
        self.stereo_mono_attention = Attention(D, kernel_size=5)
...
class DepthAggregation_wo_neighbor(nn.Module):
    ...
    def __init__(self, embed_dims=32, out_channels=1):
