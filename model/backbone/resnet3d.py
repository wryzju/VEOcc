    ...
    def __init__(
        ...
        if block_type == 'BottleNeck':
            ...
            for i in range(len(num_layer)):
                ...
                layer.extend([Bottleneck(curr_numC, curr_numC // 4, norm_cfg=norm_cfg) for _ in range(num_layer[i] - 1)])
            ...
            for i in range(len(num_layer)):
                ...
                for j in range(num_layer[i] - 1):
                    ...
                    layer.append(BasicBlock(curr_numC, curr_numC, norm_cfg=norm_cfg, block_dpr=block_dpr))
                layers.append(nn.Sequential(*layer))
        else:
            assert False
...
class CustomResNet3D(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(num_layer)):
            ...
            block_idx += 1
            layer = [
                BasicBlock3D(curr_numC,
                             num_channels[i],
                             stride=stride[i],
                             downsample=ConvModule(curr_numC,
                                                   num_channels[i],
                                                   kernel_size=3,
                                                   stride=stride[i],
                                                   padding=1,
            ...
            for _ in range(num_layer[i] - 1):
                dpr = drop_path_rate * (block_idx) / total_blocks
                block_idx += 1
                layer.append(BasicBlock3D(curr_numC, curr_numC, drop_path_rate=dpr))
            layers.append(nn.Sequential(*layer))
        self.layers = nn.Sequential(*layers)
